...
when building an :class:`~trilobite.data.core.InferenceData` object for likelihood
evaluation.
"""
...
class OpticalPhotometryContainer(DataContainer):
    ...
    See Also
    --------
    trilobite.utils.phot_utils.FilterBundle
        The filter bundle whose ``filter_names`` list defines the band→index mapping.
    trilobite.data.photometry.RadioPhotometryContainer
    ...
    def _validate_table(self, table: Table) -> Table:
        ...
        if has_flux and "flux_density_error" not in table.colnames:
            trilobite_logger.warning("Column 'flux_density' present but 'flux_density_error' is missing.")
        if has_mag and "mag_ab_error" not in table.colnames:
            trilobite_logger.warning("Column 'mag_ab' present but 'mag_ab_error' is missing.")
    ...
    def to_inference_data(
        ...
        r"""
        Convert this container to :class:`~trilobite.data.core.InferenceData`.
...
class OpticalPhotometryEpoch(DataContainer):
    r"""
    Immutable container for single-epoch, multi-band optical photometry.

    Represents broadband flux density or AB magnitude measurements taken at a
    *single epoch in time* across multiple optical bands. This container is the
    optical analog of :class:`~trilobite.data.photometry.RadioPhotometryEpoch`
    and is intended for broadband SED fitting where band is the independent
    variable.

    Observations may be expressed as **flux densities** (F_ν in erg/s/cm²/Hz),
    **AB magnitudes**, or both. Both representations are always accessible via
    properties regardless of the input format.

    Band indices are resolved at :meth:`to_inference_data` time via
    ``model.bundle.filter_names``, keeping the container independent of any
    particular model or filter ordering convention.

    Schema
    ------
    Required:

    - ``band_name`` [str] — survey band identifier (e.g. ``"g"``, ``"r"``).
    ...
    Notes
    -----
    Unlike :class:`OpticalPhotometryContainer`, there is no ``time`` column —
    all measurements are assumed to belong to the same epoch.
    ...
    COLUMNS = [
        {
            "name": "band_name",
            "dtype": str,
            "description": "Survey band identifier.",
            "required": True,
        },
        {
            "name": "flux_density",
            "dtype": float,
            "unit": "erg/(s cm2 Hz)",
            "description": "Flux density F_ν for detections.",
            "required": False,
        },
        {
            "name": "flux_density_error",
        ...
        },
        {
            ...
            "dtype": float,
            "unit": "erg/(s cm2 Hz)",
            "description": "Upper limit on F_ν for non-detections.",
            "required": False,
        },
        {
            "name": "mag_ab",
    ...
    def _validate_table(self, table: Table) -> Table:
        ...
        if not has_flux and not has_mag:
            raise ValueError(
                "OpticalPhotometryEpoch requires at least one y-column group: "
    ...
    @property
    def band_name(self) -> np.ndarray:
        """Band names as a string array."""
        return np.asarray(self.__table__["band_name"].data)

    @property
    def n_obs(self) -> int:
        """Number of bands / observations."""
        return len(self)
    ...
    @property
    def mag(self) -> np.ndarray:
        """AB magnitudes (converted from flux if needed)."""
        if self._has_mag_column:
            return np.asarray(self.__table__["mag_ab"].data, dtype=float)
    ...
    # ========================= MASKING ========================= #
    def apply_mask(self, mask) -> "OpticalPhotometryEpoch":
        """Return a new container with only the rows selected by *mask*."""
        mask = np.asarray(mask)
        return type(self)(self.__table__[mask].copy())
    ...
    def to_inference_data(
        ...
        r"""
        Convert this :class:`OpticalPhotometryEpoch` to an :class:`~trilobite.data.core.InferenceData` object.

        Band names are resolved to integer indices via ``model.bundle.filter_names``.
        Magnitudes are converted to physical flux densities before the
        InferenceData is constructed.

        Default mapping:

        - ``variables``: ``{"band_idx": <resolved integer indices>}``
        ...
        KeyError
            If a band name in the data is not found in ``model.bundle.filter_names``.
        """
        if not hasattr(model, "bundle"):
            raise AttributeError(
                f"Model '{type(model).__name__}' does not have a 'bundle' attribute. "
                "Optical photometry epoch containers require the model to expose a "
        ...
        try:
            band_idx = np.array([name_to_idx[n] for n in self.band_name], dtype=int)
        except KeyError as exc:
            bad = str(exc).strip("'")
            raise KeyError(
                f"Band name '{bad}' is not in the model's FilterBundle. Available bands: {filter_names}"
        ...
        if variables is None:
            variables = {"band_idx": idx_vals}

        if observables is None:
            if len(model.output_names) != 1:
                raise ValueError(
                    "OpticalPhotometryEpoch can only infer defaults for "
        ...
        # User-supplied observables — build a scratch table and use from_table
    ...
    def from_table(
        ...
        """Construct an :class:`OpticalPhotometryEpoch` from an Astropy table."""
        if column_map is not None:
            table = table.copy()
            table.rename_columns(list(column_map.keys()), list(column_map.values()))
        return cls(table)
    ...
    def from_file(
        ...
        **kwargs,
        ...
        """Construct an :class:`OpticalPhotometryEpoch` from a file on disk."""
