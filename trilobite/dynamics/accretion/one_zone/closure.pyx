...
:mod:`trilobite.dynamics.accretion.one_zone._sources` :
...
cdef class OneZoneClosure:
    ...
    **Opacity** — set ``self.opacity`` in ``__cinit__``.  ``_c_opacity``
    ...
    .. code-block:: python
        ...
        from trilobite.dynamics.accretion.one_zone._writer cimport (
            N_RESULT_FIELDS, standard_writer_func,
        )
        from trilobite.dynamics.accretion.one_zone.physics._viscous cimport (
    ...
    :func:`~trilobite.dynamics.accretion.one_zone.physics._fallback.fallback_source_func`
