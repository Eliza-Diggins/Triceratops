...
See Also
--------
trilobite.dynamics.shocks.core.shock_engine.ShockEngine
...
from trilobite.dynamics.shocks.core.relativistic_jump_conditions import _solve_strong_cold_shock_beta
from trilobite.dynamics.shocks.core.shock_engine import ShockEngine
from trilobite.physics_utils.constants import c_cgs, k_B_cgs, m_p_cgs
...
class RelThinShellShockState(NamedTuple):
    ...
    units to the dimensional fields; dimensionless fields (:math:`\beta`, :math:`\Gamma`, :math:`\eta`) are always
    plain arrays.
    """
...
class PressureDrivenThinShellShockEngine(ShockEngine):
    ...
    def __init__(self, mu: float = 0.5, **kwargs):
        ...
        mu : float, optional
            ...
            ionized hydrogen).
        kwargs
            Passed to :class:`~trilobite.dynamics.shocks.shock_engine.ShockEngine`.
        """
        super().__init__(**kwargs)
        self._mu = float(mu)
...
class RelPressureDrivenThinShellShockEngine(ShockEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ----------
        kwargs
            Passed to :class:`~trilobite.dynamics.shocks.core.shock_engine.ShockEngine`.
        """
    ...
    def compute_shock_properties(
        ...
        **kwargs
        ...
        Returns
        -------
        ~trilobite.dynamics.shocks.numerical.RelThinShellShockState
    ...
    def _compute_shock_properties_cgs(
        ...
        **kwargs
        ...
        # Cold-shell initial four-momentum: E = M c^2 Gamma_0, Pi = M c w_0.
...
class MechanicalShockInitialConditions(NamedTuple):
    r"""
    Self-consistent 8-component initial-condition vector for :class:`MechanicalShockEngine`.

    All values are plain CGS floats (no unit objects).  Construct this with
    :meth:`MechanicalShockEngine.infer_initial_conditions` rather than building it by hand,
    unless you have an independent estimate for each component.

    Passing an instance directly to
    :meth:`~trilobite.dynamics.shocks.numerical.MechanicalShockEngine.compute_shock_properties`
    via the ``initial_conditions`` keyword is the recommended workflow.
    """

    R_cd_0: float
    r"""Contact-discontinuity radius at :math:`t_0` in cm."""

    v_cd_0: float
    r"""Contact-discontinuity velocity at :math:`t_0` in cm/s."""

    M2_0: float
    r"""Shocked ejecta mass at :math:`t_0` in g."""

    M3_0: float
    r"""Shocked CSM mass at :math:`t_0` in g."""

    U2_0: float
    r"""Shocked ejecta internal energy at :math:`t_0` in erg."""

    U3_0: float
    r"""Shocked CSM internal energy at :math:`t_0` in erg."""

    Delta2_0: float
    r"""Effective shocked ejecta layer width at :math:`t_0` in cm."""

    Delta3_0: float
    r"""Effective shocked CSM layer width at :math:`t_0` in cm."""
...
class MechanicalShockState(NamedTuple):
    ...
    :math:`P_2 = (\gamma_2-1)U_2/V_2` in :math:`\mathrm{dyn\,cm^{-2}}`.
    """

    pressure_3: Union[np.ndarray, u.Quantity]
    r"""
    Volume-averaged shocked-CSM pressure
    :math:`P_3 = (\gamma_3-1)U_3/V_3` in :math:`\mathrm{dyn\,cm^{-2}}`.
    """
    ...
    # ------------------------------------------------------------------ #
    post_shock_density_rs: Union[np.ndarray, u.Quantity]
    r"""
    Immediate post-shock density at the reverse shock in :math:`\mathrm{g\,cm^{-3}}`.

    This is an instantaneous Rankine--Hugoniot jump diagnostic, not the
    volume-averaged density of Region 2.
    """

    post_shock_pressure_rs: Union[np.ndarray, u.Quantity]
    r"""
    Immediate post-shock pressure at the reverse shock in :math:`\mathrm{dyn\,cm^{-2}}`.

    This is an instantaneous Rankine--Hugoniot jump diagnostic, not the
    volume-averaged pressure of Region 2.
    """

    post_shock_temperature_rs: Union[np.ndarray, u.Quantity]
    r"""
    ...
    post_shock_thermal_energy_density_rs: Union[np.ndarray, u.Quantity]
    r"""
    Immediate post-shock thermal energy density at the reverse shock in :math:`\mathrm{erg\,cm^{-3}}`.
    """

    # ------------------------------------------------------------------ #
    # Instantaneous forward-shock jump diagnostics                        #
    # ------------------------------------------------------------------ #
    post_shock_density_fs: Union[np.ndarray, u.Quantity]
    r"""
    Immediate post-shock density at the forward shock in :math:`\mathrm{g\,cm^{-3}}`.

    This is an instantaneous Rankine--Hugoniot jump diagnostic, not the
    volume-averaged density of Region 3.
    """

    post_shock_pressure_fs: Union[np.ndarray, u.Quantity]
    r"""
    Immediate post-shock pressure at the forward shock in :math:`\mathrm{dyn\,cm^{-2}}`.

    This is an instantaneous Rankine--Hugoniot jump diagnostic, not the
    volume-averaged pressure of Region 3.
    """

    post_shock_temperature_fs: Union[np.ndarray, u.Quantity]
    r"""
    ...
    post_shock_thermal_energy_density_fs: Union[np.ndarray, u.Quantity]
    r"""
    Immediate post-shock thermal energy density at the forward shock in :math:`\mathrm{erg\,cm^{-3}}`.
    """


class MechanicalShockEngine(ShockEngine):
    r"""
    Non-relativistic mechanical shock engine tracking forward and reverse shock evolution.

    This class implements a **non-relativistic mechanical shock model** for ejecta--CSM
    interaction. This model resolves the shock structure into 4 distinct regions:

    1. **Region 1**: Unshocked ejecta, with density profile :math:`\rho_{\rm ej}(r,t)`.
    2. **Region 2**: Shocked ejecta, with volume-averaged density :math:`\bar{\rho}_2(t)`,
       pressure :math:`P_2(t)`, and temperature :math:`T_2(t)`.
    3. **Region 3**: Shocked CSM, with volume-averaged density :math:`\bar{\rho}_3(t)`,
       pressure :math:`P_3(t)`, and temperature :math:`T_3(t)`.
    4. **Region 4**: Unshocked CSM, with density profile :math:`\rho_{\rm csm}(r)`

    Each shocked layer is assigned its own swept-up mass,
    internal energy, effective volume, and layer-averaged pressure. The model follows
    the general spirit of the mechanical treatment in
    :footcite:t:`beloborodovMechanicalModelRelativistic2006a` and the simplified
    internal-energy closure described by
    :footcite:t:`wangVegasAfterglowHighperformanceFramework2026`, specialized here
    to the non-relativistic case.

    .. rubric:: Initial Conditions

    The following are the initial conditions necessary to run the mechanical shock engine:
    ...
    .. list-table::
        ...
        * - Parameter
          - Meaning
        * - :math:`R_{{\rm cd},0}`
          - The initial position of the contact discontinuity (CD) between the shocked ejecta and shocked CSM.
        * - :math:`v_{{\rm cd},0}`
          - Initial contact-discontinuity velocity.
        * - :math:`M_{2,0}`
          - Initial mass in the shocked ejecta layer.
        * - :math:`M_{3,0}`
          - Initial mass in the shocked CSM layer.
        * - :math:`U_{2,0}`
          - Initial internal energy of the shocked ejecta layer.
        * - :math:`U_{3,0}`
          - Initial internal energy of the shocked CSM layer.
        * - :math:`\Delta_{2,0}`
          - Initial effective width of the shocked ejecta layer.
        * - :math:`\Delta_{3,0}`
          - Initial effective width of the shocked CSM layer.
        * - :math:`M_{\rm ej}`
          - Optional finite ejecta-mass scale used to determine when the reverse shock has crossed the ejecta.

    The adiabatic indices :math:`\gamma_2`, :math:`\gamma_3` and mean molecular weights
    :math:`\mu_2`, :math:`\mu_3` are engine constructor parameters, not initial conditions;
    set them via ``MechanicalShockEngine(gamma_2=..., gamma_3=...)``.  The 8 scalar initial
    conditions above can be derived self-consistently from :math:`(R_{{\rm cd},0},\,v_{{\rm cd},0},\,t_0)`
    using :meth:`~trilobite.dynamics.shocks.numerical.MechanicalShockEngine.infer_initial_conditions`.

    .. rubric:: Model Outputs

    The mechanical shock engine returns a
    :class:`~trilobite.dynamics.shocks.numerical.MechanicalShockState`
    containing the evolved shock dynamics, shocked-layer properties, and
    instantaneous Rankine--Hugoniot jump-condition diagnostics.

    .. dropdown:: Output Fields
        ...
        .. list-table::
            ...
            * - ``mass_2``
              - Swept-up mass in the shocked ejecta layer, Region 2.
            * - ``mass_3``
              - Swept-up mass in the shocked CSM layer, Region 3.
            * - ``energy_2``
              - Internal energy of the shocked ejecta layer, :math:`U_2(t)`.
            * - ``energy_3``
              - Internal energy of the shocked CSM layer, :math:`U_3(t)`.
            * - ``width_2``
              - Effective width of the shocked ejecta layer, :math:`\Delta_2(t)`.
            * - ``width_3``
              - Effective width of the shocked CSM layer, :math:`\Delta_3(t)`.
            * - ``radius_rs``
              - Reverse-shock radius, :math:`R_{\rm rs}=R_{\rm cd}-\Delta_2`. After ejecta crossing, this is reported
                as zero.
            * - ``radius_fs``
              - Forward-shock radius, :math:`R_{\rm fs}=R_{\rm cd}+\Delta_3`.
            * - ``velocity_rs``
              - Reverse-shock velocity, :math:`D_{\rm rs}`. After ejecta crossing,
                this is reported as ``NaN`` because the reverse shock is no longer active.
            * - ``velocity_fs``
              - Forward-shock velocity, :math:`D_{\rm fs}`.
            * - ``volume_2``
              - Effective volume of the shocked ejecta layer, :math:`V_2\simeq4\pi R_{\rm cd}^2\Delta_2`.
            * - ``volume_3``
              - Effective volume of the shocked CSM layer, :math:`V_3\simeq4\pi R_{\rm cd}^2\Delta_3`.
            * - ``density_2``
              - Layer-averaged density of the shocked ejecta, :math:`\bar{\rho}_2=M_2/V_2`.
            * - ``density_3``
              - Layer-averaged density of the shocked CSM, :math:`\bar{\rho}_3=M_3/V_3`.
            * - ``pressure_2``
              - Layer-averaged pressure of the shocked ejecta, :math:`P_2=(\gamma_2-1)U_2/V_2`.
            * - ``pressure_3``
              - Layer-averaged pressure of the shocked CSM, :math:`P_3=(\gamma_3-1)U_3/V_3`.
            * - ``temperature_2``
              - Layer-averaged shocked-ejecta temperature inferred from :math:`P_2/\bar{\rho}_2`.
            * - ``temperature_3``
              - Layer-averaged shocked-CSM temperature inferred from :math:`P_3/\bar{\rho}_3`.
            * - ``thermal_energy_density_2``
              - Layer-averaged thermal energy density of the shocked ejecta, :math:`U_2/V_2`.
            * - ``thermal_energy_density_3``
              - Layer-averaged thermal energy density of the shocked CSM, :math:`U_3/V_3`.
            * - ``sound_speed_2``
              - Effective sound speed in the shocked ejecta layer, :math:`c_{s,2}=\sqrt{\gamma_2 P_2/\bar{\rho}_2}`.
            * - ``sound_speed_3``
              - Effective sound speed in the shocked CSM layer, :math:`c_{s,3}=\sqrt{\gamma_3 P_3/\bar{\rho}_3}`.
            * - ``post_shock_density_rs``
              - Instantaneous Rankine--Hugoniot downstream density immediately behind the reverse shock.
                Reported as ``NaN`` when the reverse shock is inactive.
            * - ``post_shock_pressure_rs``
              - Instantaneous Rankine--Hugoniot downstream pressure immediately behind the reverse shock.
                Reported as ``NaN`` when the reverse shock is inactive.
            * - ``post_shock_temperature_rs``
              - Instantaneous Rankine--Hugoniot downstream temperature immediately behind the reverse shock.
                Reported as ``NaN`` when the reverse shock is inactive.
            * - ``post_shock_thermal_energy_density_rs``
              - Instantaneous Rankine--Hugoniot downstream thermal energy density immediately behind the
                reverse shock. Reported as ``NaN`` when the reverse shock is inactive.
            * - ``post_shock_density_fs``
              - Instantaneous Rankine--Hugoniot downstream density immediately behind the forward shock.
                Reported as ``NaN`` when the forward shock is inactive.
            * - ``post_shock_pressure_fs``
              - Instantaneous Rankine--Hugoniot downstream pressure immediately behind the forward shock.
                Reported as ``NaN`` when the forward shock is inactive.
            * - ``post_shock_temperature_fs``
              - Instantaneous Rankine--Hugoniot downstream temperature immediately behind the forward shock.
                Reported as ``NaN`` when the forward shock is inactive.
            * - ``post_shock_thermal_energy_density_fs``
              - Instantaneous Rankine--Hugoniot downstream thermal energy density immediately behind the
                forward shock. Reported as ``NaN`` when the forward shock is inactive.


    .. rubric:: Model Equations
    ...
    where :math:`R_{\rm cd}` and :math:`v_{\rm cd}` are the contact-discontinuity
    radius and velocity, :math:`M_i` and :math:`U_i` are the mass and internal
    energy of shocked layer :math:`i`, and :math:`\Delta_i` is the effective
    single-zone width of that layer. Region 2 lies between the reverse shock and the
    contact discontinuity, while Region 3 lies between the contact discontinuity and
    the forward shock:
    ...
    The shocked-layer volumes are approximated by thin-slab expressions,

    .. math::

        V_i \simeq 4\pi R_{\rm cd}^2\Delta_i,

    and the pressures are computed from the evolved internal energies,

    .. math::

        P_i = (\gamma_i - 1)\frac{U_i}{V_i}.

    .. important::

        The pressures :math:`P_2` and :math:`P_3` are **not** instantaneous
        Rankine--Hugoniot post-shock pressures. They are layer-averaged pressures
        determined by the internal-energy budgets of the shocked layers. They evolve
        continuously as mass is swept up, shock heating is deposited, radiative
        losses are applied, and expansion work is done.
    ...
    are the reverse- and forward-shock speeds.

    The contact-discontinuity acceleration is driven by the layer pressure imbalance,
    ...
    The internal-energy equation for each shocked layer is
    ...
    where :math:`\dot{U}_{{\rm rad},i}` is an optional user-specified radiative-loss
    term. For a cold, strong, non-relativistic shock, the compression ratio is

    .. math::

        \chi_i = \frac{\gamma_i+1}{\gamma_i-1},

    and the shock-heating source is
    ...
    The adiabatic expansion term follows from :math:`P\,dV` work:
    ...
    .. math::
        ...
        -(\gamma_i - 1)U_i
        \left(
            \frac{2v_{\rm cd}}{R_{\rm cd}}
            +
            \frac{c_{s,i}}{\Delta_i}
        \right).

    The effective layer widths evolve at the layer-averaged sound speed,
    ...
    .. math::
        ...
        =
        \sqrt{
            \gamma_i(\gamma_i - 1)\frac{U_i}{M_i}
        }.

    Once the reverse shock has crossed the ejecta (:math:`M_2 \to M_{\rm ej}`),
    the model sets :math:`\dot{M}_2=0` and :math:`\dot{U}_{{\rm sh},2}=0`; the
    sound-speed width closure for :math:`\Delta_2` continues unchanged.

    If the reverse-shock relative velocity
    :math:`u_1(R_{\rm rs},t) - D_{\rm rs} \le 0` at any step, the associated
    mass-loading and shock-heating terms are set to zero.

    .. note::

        For a more detailed description of the theory behind this engine, see
    ...
    :ref:`mechanical_internal_energy_model`
        Theory derivation.
    :class:`~trilobite.dynamics.shocks.numerical.PressureDrivenThinShellShockEngine`
        Simpler thin-shell closure with algebraic shock pressures.
    ...
    # Fraction of M1_total at which the reverse shock is treated as fully traversed.
    _EJECTA_MASS_TOL: float = 1e-4

    # Default ODE relative tolerance. 1e-6 balances MCMC speed against accuracy;
    # tighten to 1e-10 for publication figures or precise energy-conservation checks.
    _DEFAULT_RTOL: float = 1e-6

    # Maximum initial layer width as a fraction of R_cd in the sound-crossing IC.
    _MAX_DELTA_FRAC: float = 0.45
    ...
    def __init__(
        ...
        **kwargs,
        ...
        gamma_2 : float, optional
            Adiabatic index of the shocked ejecta (Region 2).  Default ``5/3``
            (non-relativistic monatomic ideal gas).
        gamma_3 : float, optional
            Adiabatic index of the shocked CSM (Region 3).  Default ``5/3``.
        kwargs
            Passed to the base :class:`~trilobite.dynamics.shocks.core.shock_engine.ShockEngine`.
        """
        super().__init__(**kwargs)
        self._mu_2 = float(mu_2)
        self._mu_3 = float(mu_3)
        self._gamma_2 = float(gamma_2)
        self._gamma_3 = float(gamma_3)

        # Pre-compute compression ratios and post-shock sound-speed coefficients.
        # chi_i = (gamma_i + 1) / (gamma_i - 1) is the strong-shock compression ratio.
        # beta_i = sqrt(gamma_i * (chi_i - 1) / chi_i^2) is the ratio c_{s,i} / v_{rel,i}
        # in the strong-shock limit (used for the sound-crossing IC).
        self._chi_2 = (gamma_2 + 1.0) / (gamma_2 - 1.0)
        self._chi_3 = (gamma_3 + 1.0) / (gamma_3 - 1.0)
        self._beta_2 = float(np.sqrt(gamma_2 * (self._chi_2 - 1.0) / self._chi_2**2))
        self._beta_3 = float(np.sqrt(gamma_3 * (self._chi_3 - 1.0) / self._chi_3**2))

    # ------------------------------------------------------------------ #
    # Initial Conditions and Evaluation Kernel Generation                #
    # ------------------------------------------------------------------ #
    def infer_initial_conditions(
        self,
        ...
        u_1: "Callable",
        u_4: "Callable",
    ) -> "MechanicalShockInitialConditions":
        r"""
        Derive a self-consistent 8-component initial-condition vector.

        Given the contact-discontinuity position and velocity at time :math:`t_0`,
        this method constructs
        :math:`(R_{{\rm cd},0},\,v_{{\rm cd},0},\,M_{2,0},\,M_{3,0},\,U_{2,0},\,U_{3,0},\,\Delta_{2,0},\,\Delta_{3,0})`
        such that the ODE kernel is free of artificial relaxation transients at :math:`t = t_0`.
        The adiabatic indices :math:`\gamma_2`, :math:`\gamma_3` are taken from the engine instance.

        **Sound-crossing width estimate.**
        Each layer width is initialised to the distance sound would have travelled across
        that layer since the explosion:

        .. math::

            \Delta_{i,0} = c_{s,i,0}\,t_0,

        where :math:`c_{s,i,0} = \beta_i\,v_{{\rm rel},i,0}` is the post-shock sound speed
        evaluated at :math:`R_{\rm cd}` (one-iteration approximation).  The coefficient
        :math:`\beta_i = \sqrt{\gamma_i(\chi_i-1)/\chi_i^2}` is the strong-shock ratio
        :math:`c_s/v_{\rm rel}`.  For a self-similar shock with shock speed decelerating as
        :math:`v_{\rm sh}\propto t^{m-1}`, this estimate overestimates the true width by a factor
        :math:`1/m`, which is at most ~15% for the :math:`n=10`, :math:`s=2` Chevalier solution
        (:math:`m=7/8`).  The ODE integrator corrects this small discrepancy within a few sound
        crossing times.  Both widths are capped at :math:`0.45\,R_{{\rm cd},0}` to keep the
        shock faces within the thin-shell regime.

        **Swept-up mass integrals.**
        With the initial shock-face positions :math:`R_{{\rm rs},0}=R_{{\rm cd},0}-\Delta_{2,0}` and
        :math:`R_{{\rm fs},0}=R_{{\rm cd},0}+\Delta_{3,0}` determined, the swept-up masses are
        ...
        :math:`M_{2,0}` is integrated in :math:`\ln r` to avoid catastrophic cancellation for
        steep broken-power-law outer profiles.

        **Self-consistent relative velocities and internal energies.**
        The relative velocities at the actual shock faces are then computed from each closure:

        .. math::

            v_{{\rm rel},2,0} = \frac{\chi_2}{\chi_2-1}
                \bigl(u_1(R_{{\rm rs},0},\,t_0) - v_{{\rm cd},0}\bigr),
            \qquad
            v_{{\rm rel},3,0} = \frac{v_{{\rm cd},0} - u_4(R_{{\rm fs},0},\,t_0)}{1 - \beta_3}.

        The Rankine--Hugoniot specific internal energies follow as

        .. math::

            U_{i,0} = \frac{\chi_i-1}{(\gamma_i-1)\chi_i^2}\,v_{{\rm rel},i,0}^2\,M_{i,0}.
        ...
        Returns
        -------
        ~trilobite.dynamics.shocks.numerical.MechanicalShockInitialConditions
            Named tuple of CGS floats, ready to pass directly to
            :meth:`compute_shock_properties` via the ``initial_conditions`` keyword.

        Raises
        ------
        ValueError
            If the upstream conditions at :math:`R_{{\rm cd},0}` imply a
            non-positive shock-frame relative velocity for either shock after the
            width estimate, meaning no valid shock exists at the chosen position
            and velocity.

        Notes
        -----
        Both mass integrals are evaluated with :func:`scipy.integrate.quad`
        (adaptive Gauss--Kronrod quadrature, ``limit=100`` subintervals).  The
        :math:`M_2` upper limit is capped at :math:`\ln R_{{\rm rs},0} + 100`
        (roughly 43 orders of magnitude in radius); no physical ejecta profile
        carries appreciable mass beyond this point.  Because ``quad`` evaluates
        the integrand at scalar values, each density callable is invoked with a
        1-element NumPy array and the result extracted with ``.flat[0]``, keeping
        callables that use boolean array masking internally compatible.
        """
        # --- strip units ---
        ...
        beta_2 = self._beta_2
        beta_3 = self._beta_3
        gamma_2 = self._gamma_2
        gamma_3 = self._gamma_3

        # --- sound-crossing width estimate ---
        # Approximate v_rel at R_cd_0 (one iteration; error is O(Delta/R_cd)).
        # RS: v_rel_2 = (u1 - v_cd) * chi_2 / (chi_2 - 1)  [from velocity-ratio closure]
        # FS: v_rel_3 = (v_cd - u4) / (1 - beta_3)          [from sound-speed closure]
        u1_at_cd = float(np.asarray(u_1(R_cd_0, t_0)).flat[0])
        u4_at_cd = float(np.asarray(u_4(R_cd_0, t_0)).flat[0])

        u1_rel_approx = u1_at_cd - v_cd_0
        u4_rel_approx = v_cd_0 - u4_at_cd

        if u1_rel_approx > 0.0:
            v_rel_2_approx = u1_rel_approx * chi_2 / (chi_2 - 1.0)
            Delta2_0 = min(beta_2 * v_rel_2_approx * t_0, self._MAX_DELTA_FRAC * R_cd_0)
        else:
            Delta2_0 = 0.01 * R_cd_0

        if u4_rel_approx > 0.0:
            v_rel_3_approx = u4_rel_approx / (1.0 - beta_3)
            Delta3_0 = min(beta_3 * v_rel_3_approx * t_0, self._MAX_DELTA_FRAC * R_cd_0)
        else:
            Delta3_0 = 0.01 * R_cd_0

        # --- verify RS face is in the supersonic ejecta region ---
        # For profiles where the sound-crossing estimate is too large (e.g. s=0
        # uniform ISM where m=0.7 limits the RS to the outer 30% of R_cd), the
        # initial Delta2_0 may place R_rs_0 below the CD velocity.  Bisect
        # Delta2_0 until u_1(R_rs_0, t_0) > v_cd_0 (up to 30 halvings, i.e.
        # 10^9 reduction, which always finds a valid position for physical ejecta).
        for _ in range(30):
            R_rs_0_trial = R_cd_0 - Delta2_0
            u1_trial = float(np.asarray(u_1(np.array([R_rs_0_trial]), t_0)).flat[0])
            if u1_trial > v_cd_0:
                break
            Delta2_0 *= 0.5
        else:
            raise ValueError(
                "Cannot place the reverse-shock face in the supersonic ejecta region "
                "after 30 bisection steps. Verify that u_1(r, t_0) > v_cd_0 for some "
                "r < R_cd_0 (i.e. that a reverse shock physically exists at t_0)."
            )
        ...
        # M2: integrated in log-r to handle steep BPL outer profiles (see docstring).
        def _log_M2_integrand(log_r: float) -> float:
            r = np.exp(log_r)
            val = 4.0 * np.pi * r**3 * np.asarray(rho_1(np.array([r]), t_0)).flat[0]
            return val if np.isfinite(val) else 0.0

        M2_0, _ = quad(_log_M2_integrand, np.log(R_rs_0), np.log(R_rs_0) + 100.0, limit=100)
        ...
        # --- self-consistent v_rel at the actual shock face positions ---
        v_rel_2_0 = (float(np.asarray(u_1(np.array([R_rs_0]), t_0)).flat[0]) - v_cd_0) * chi_2 / (chi_2 - 1.0)
        v_rel_3_0 = (v_cd_0 - float(np.asarray(u_4(np.array([R_fs_0]), t_0)).flat[0])) / (1.0 - beta_3)

        if v_rel_2_0 <= 0:
            raise ValueError(
                f"Reverse shock has non-positive upstream relative velocity "
                f"({v_rel_2_0:.3e} cm/s) even after bisection. "
                f"Ensure u_1(r, t_0) > v_cd_0 for some r < R_cd_0."
            )
        if v_rel_3_0 <= 0:
            raise ValueError(
        ...
        eps_2 = ((chi_2 - 1.0) / ((gamma_2 - 1.0) * chi_2**2)) * v_rel_2_0**2
        eps_3 = ((chi_3 - 1.0) / ((gamma_3 - 1.0) * chi_3**2)) * v_rel_3_0**2

        U2_0 = eps_2 * M2_0
        U3_0 = eps_3 * M3_0

        return MechanicalShockInitialConditions(
            R_cd_0=R_cd_0,
            v_cd_0=v_cd_0,
            M2_0=M2_0,
            M3_0=M3_0,
    ...
    @classmethod
    def find_ejecta_crossing_time(
        cls,
        time: "_UnitBearingArrayLike",
        state: "MechanicalShockState",
        M1_total: "_UnitBearingScalarLike",
    ) -> "Union[u.Quantity, None]":
        r"""
        Return the first time at which the reverse shock has crossed all ejecta.

        The crossing is defined as the first output step where

        .. math::

            M_2 \geq (1 - \epsilon_{\rm tol})\,M_{\rm ej},

        where :math:`\epsilon_{\rm tol} = 10^{-4}` (:attr:`_EJECTA_MASS_TOL`).

        Parameters
        ----------
        time : ~astropy.units.Quantity or array-like
            Times corresponding to the rows of ``state``. Unit-bearing inputs
            are converted to seconds; bare floats are assumed to be in seconds.
        state : ~trilobite.dynamics.shocks.numerical.MechanicalShockState
            State returned by :meth:`compute_shock_properties`.
        M1_total : ~astropy.units.Quantity or float
            Total ejecta mass. Unit-bearing inputs are converted to g; bare
            floats are assumed to be in grams.

        Returns
        -------
        ~astropy.units.Quantity or None
            Time of ejecta crossing in seconds, or ``None`` if the condition
            is never satisfied within the supplied time grid.
        """
        time_arr = time.to(u.s).value if isinstance(time, u.Quantity) else np.asarray(time, dtype=float)
        M1 = float(ensure_in_units(M1_total, u.g))
        mass_2 = (
            state.mass_2.to(u.g).value
            if isinstance(state.mass_2, u.Quantity)
            else np.asarray(state.mass_2, dtype=float)
        )
        threshold = (1.0 - cls._EJECTA_MASS_TOL) * M1
        idx = np.searchsorted(mass_2, threshold)
        if idx >= len(time_arr):
            return None
        return time_arr[idx] * u.s
    ...
    def generate_evaluation_kernel(
        ...
        Returns a function ``kernel(t, y)`` suitable for
        :func:`scipy.integrate.solve_ivp`.  Adiabatic indices are taken from the
        engine instance (``self._gamma_2``, ``self._gamma_3``).
        ...
        callable
            ``kernel(t, y) -> dy/dt`` where
            ``y = [R_cd, Phi, M2, M3, U2, U3, Delta2, Delta3]``
            and ``Phi = v_cd^2``.
        """
        # Pull instance constants into the local scope -- avoids repeated
        # attribute lookups inside the hot kernel path.
        gamma_2 = self._gamma_2
        gamma_3 = self._gamma_3
        chi_2 = self._chi_2
        chi_3 = self._chi_3
        _EJECTA_MASS_TOL = self._EJECTA_MASS_TOL

        # Pre-compute scalar factors used every kernel call.
        gamma_2m1 = gamma_2 - 1.0
        gamma_3m1 = gamma_3 - 1.0
        chi_2m1 = chi_2 - 1.0
        # Shock-heating coefficient: (chi-1) / (chi^2 * (gamma-1))
        _sh_coeff2 = chi_2m1 / (chi_2 * chi_2 * gamma_2m1)
        _sh_coeff3 = (chi_3 - 1.0) / (chi_3 * chi_3 * gamma_3m1)
        _4pi = 4.0 * np.pi

        _has_cooling_2 = cooling_2 is not None
        _has_cooling_3 = cooling_3 is not None
        _M1_total = np.inf if M1_total is None else float(M1_total)

        def _kernel(t, y):
            R_cd, Phi, M2, M3, U2, U3, Delta2, Delta3 = y

            # CD velocity from Phi = v_cd^2; guard against tiny numerical negatives.
            v_cd = Phi**0.5 if Phi > 0.0 else 0.0

            # Floor internal energies so pressures and sound speeds remain non-negative.
            # Under strong cooling U can drift below zero; clamping here prevents
            # negative pressures from entering the EOS without changing the ODE state.
            U2_eff = U2 if U2 > 0.0 else 0.0
            U3_eff = U3 if U3 > 0.0 else 0.0

            # Floor layer widths to prevent the 1/Delta singularity in the adiabatic
            # expansion term. Delta2 is also capped at R_cd: once the RS has crossed
            # all ejecta, Region 2 fills the sphere [0, R_cd] and Delta2 = R_cd.
            _min_d = 1.0e-10 * R_cd
            Delta2_s = max(min(Delta2, R_cd), _min_d)
            Delta3_s = Delta3 if Delta3 > _min_d else _min_d

            # Shock face positions; R_rs >= 0 by construction (Delta2 <= R_cd above).
            R_rs = R_cd - Delta2_s
            R_fs = R_cd + Delta3_s

            # EOS: pressures and sound speeds from floored energies.
            _4pi_Rcd2 = _4pi * R_cd * R_cd
            _cs2_sq = gamma_2 * gamma_2m1 * U2_eff / M2 if M2 > 0.0 else 0.0
            _cs3_sq = gamma_3 * gamma_3m1 * U3_eff / M3 if M3 > 0.0 else 0.0
            c_s3 = _cs3_sq**0.5 if _cs3_sq > 0.0 else 0.0

            # Upstream quantities at shock faces.
            rho1_rs = float(rho_1(R_rs, t))
            rho4_fs = float(rho_4(R_fs, t))
            u1_rs = float(u_1(R_rs, t))
            u4_fs = float(u_4(R_fs, t))

            # Ejecta-crossing switch: Python if avoids evaluating both branches
            # every call (unlike np.where) and signals to Radau that the RHS is
            # piecewise smooth between events.
            if M2 >= (1.0 - _EJECTA_MASS_TOL) * _M1_total:
                # RS has swept all ejecta; Region 2 now fills the sphere [0, R_cd].
                # Track the CD velocity exactly so Delta2 = R_cd remains satisfied
                # and R_rs = R_cd - Delta2 = 0 for all subsequent steps.
                # The adiabatic term then gives -3*(gamma-1)*U2*v_cd/R_cd, the correct
                # sphere expansion rate (U ~ R_cd^{-2} for gamma=5/3).
                dwidth2_dt = v_cd
                dM2_dt = 0.0
                dU_sh2 = 0.0
            else:
                u1_rel = u1_rs - v_cd
                _dw2 = u1_rel / chi_2m1
                dwidth2_dt = _dw2 if _dw2 > 0.0 else 0.0
                D_rs = v_cd - dwidth2_dt
                v_rel_2 = u1_rs - D_rs
                if v_rel_2 < 0.0:
                    v_rel_2 = 0.0
                dM2_dt = _4pi * R_rs * R_rs * rho1_rs * v_rel_2
                dU_sh2 = _sh_coeff2 * v_rel_2 * v_rel_2 * dM2_dt

            # Forward shock (always active while CSM exists).
            dwidth3_dt = c_s3
            D_fs = v_cd + dwidth3_dt
            v_rel_3 = D_fs - u4_fs
            if v_rel_3 < 0.0:
                v_rel_3 = 0.0
            dM3_dt = _4pi * R_fs * R_fs * rho4_fs * v_rel_3
            dU_sh3 = _sh_coeff3 * v_rel_3 * v_rel_3 * dM3_dt

            # Adiabatic expansion losses (PdV via V_i = 4*pi*R_cd^2*Delta_i).
            _2vcd_Rcd = 2.0 * v_cd / R_cd
            dU_ad2 = -gamma_2m1 * U2_eff * (_2vcd_Rcd + dwidth2_dt / Delta2_s)
            dU_ad3 = -gamma_3m1 * U3_eff * (_2vcd_Rcd + dwidth3_dt / Delta3_s)

            # Radiative losses (branch on bool to skip the call when not needed).
            dU_rad2 = cooling_2(R_cd, v_cd, M2, U2, Delta2, t) if _has_cooling_2 else 0.0
            dU_rad3 = cooling_3(R_cd, v_cd, M3, U3, Delta3, t) if _has_cooling_3 else 0.0

            dU2_dt = dU_sh2 + dU_ad2 + dU_rad2
            dU3_dt = dU_sh3 + dU_ad3 + dU_rad3

            # Energy-conserving Phi = v_cd^2 equation.
            # Radiative losses cancel here (they reduce U_i, which reduces the
            # pressure gradient driving the CD, but do not directly alter E_kin).
            E_adv = 0.5 * (u1_rs * u1_rs * dM2_dt + u4_fs * u4_fs * dM3_dt)
            M_total = M2 + M3 if (M2 + M3) > 0.0 else 1.0e-30
            dM_dt = dM2_dt + dM3_dt
            dPhi_dt = (2.0 / M_total) * (E_adv - 0.5 * dM_dt * Phi - (dU_sh2 + dU_ad2) - (dU_sh3 + dU_ad3))

            return np.array([v_cd, dPhi_dt, dM2_dt, dM3_dt, dU2_dt, dU3_dt, dwidth2_dt, dwidth3_dt])
    ...
    def compute_shock_properties(
        ...
        u_1: Callable,
        u_4: Callable,
        initial_conditions: "Union[MechanicalShockInitialConditions, None]" = None,
        ...
        **kwargs,
        ...
        Adiabatic indices are taken from the engine instance (``self._gamma_2``,
        ``self._gamma_3``); set them at instantiation with
        ``MechanicalShockEngine(gamma_2=..., gamma_3=...)``.
        ...
        initial_conditions : ~trilobite.dynamics.shocks.numerical.MechanicalShockInitialConditions or None, optional
            Self-consistent IC vector returned by :meth:`infer_initial_conditions`.
            When provided, all individual ``R_cd_0``, ``v_cd_0``, ... ``Delta3_0``
            keyword arguments are ignored.  The values are plain CGS floats (no unit
            stripping needed).  Default ``None``.
        R_cd_0 : ~astropy.units.Quantity or float, optional
            Initial contact-discontinuity radius. Used only when
            ``initial_conditions`` is ``None``. Default ``1e14 cm``.
        v_cd_0 : ~astropy.units.Quantity or float, optional
            Initial contact-discontinuity velocity. Default ``1e9 cm/s``.
        M2_0 : ~astropy.units.Quantity or float, optional
            Initial shocked ejecta mass. Default ``1e26 g``.
        M3_0 : ~astropy.units.Quantity or float, optional
            Initial shocked CSM mass. Default ``1e26 g``.
        U2_0 : ~astropy.units.Quantity or float, optional
            Initial shocked ejecta internal energy. Default ``1e45 erg``.
        U3_0 : ~astropy.units.Quantity or float, optional
            Initial shocked CSM internal energy. Default ``1e45 erg``.
        Delta2_0 : ~astropy.units.Quantity or float, optional
            Initial shocked ejecta layer width. Default ``1e12 cm``.
        Delta3_0 : ~astropy.units.Quantity or float, optional
            Initial shocked CSM layer width. Default ``1e12 cm``.
        t_0 : ~astropy.units.Quantity or float, optional
            Initial time. Default ``1.0 s``.
        cooling_2 : callable or None, optional
        ...
        M1_total : ~astropy.units.Quantity or float or None, optional
            ...
            An ODE event is used to restart integration cleanly at the crossing point.
            Default ``None`` (no limit).
        **kwargs
            Forwarded to :func:`scipy.integrate.solve_ivp`.
            ``method`` defaults to ``'Radau'``; ``rtol`` defaults to
            ``1e-6`` (suitable for parameter studies; tighten to ``1e-10`` for
            publication accuracy).

        Returns
        -------
        ~trilobite.dynamics.shocks.numerical.MechanicalShockState
            Named tuple of :class:`~astropy.units.Quantity` arrays.
        """
        time_s = time.to(u.s).value if isinstance(time, u.Quantity) else np.asarray(time, dtype=float)

        if initial_conditions is not None:
            # IC from infer_initial_conditions: already plain CGS floats, no stripping needed.
            ic = initial_conditions
            R_cd_0_cgs = float(ic.R_cd_0)
            v_cd_0_cgs = float(ic.v_cd_0)
            M2_0_cgs = float(ic.M2_0)
            M3_0_cgs = float(ic.M3_0)
            U2_0_cgs = float(ic.U2_0)
            U3_0_cgs = float(ic.U3_0)
            Delta2_0_cgs = float(ic.Delta2_0)
            Delta3_0_cgs = float(ic.Delta3_0)
            t_0_s = float(ensure_in_units(t_0, u.s))
        else:
            R_cd_0_cgs = float(ensure_in_units(R_cd_0, u.cm))
            v_cd_0_cgs = float(ensure_in_units(v_cd_0, u.cm / u.s))
            M2_0_cgs = float(ensure_in_units(M2_0, u.g))
            M3_0_cgs = float(ensure_in_units(M3_0, u.g))
            U2_0_cgs = float(ensure_in_units(U2_0, u.erg))
            U3_0_cgs = float(ensure_in_units(U3_0, u.erg))
            Delta2_0_cgs = float(ensure_in_units(Delta2_0, u.cm))
            Delta3_0_cgs = float(ensure_in_units(Delta3_0, u.cm))
            t_0_s = float(ensure_in_units(t_0, u.s))

        M1_total_cgs = (
            None
            if M1_total is None
            else float(ensure_in_units(M1_total, u.g))
            if isinstance(M1_total, u.Quantity)
            else float(M1_total)
        )
        ...
        cgs = self._compute_shock_properties_cgs(
            ...
            U2_0=U2_0_cgs,
            U3_0=U3_0_cgs,
            Delta2_0=Delta2_0_cgs,
            Delta3_0=Delta3_0_cgs,
            t_0=t_0_s,
            cooling_2=cooling_2,
            cooling_3=cooling_3,
            M1_total=M1_total_cgs,
            **kwargs,
        )

        _cm3 = u.cm**3
        _dyn_cm2 = u.dyn / u.cm**2
        _erg_cm3 = u.erg / u.cm**3
        _g_cm3 = u.g / u.cm**3
        _cm_s = u.cm / u.s

        return MechanicalShockState(
            radius=cgs.radius * u.cm,
            velocity=cgs.velocity * _cm_s,
            ...
            velocity_rs=cgs.velocity_rs * _cm_s,
            velocity_fs=cgs.velocity_fs * _cm_s,
            volume_2=cgs.volume_2 * _cm3,
            volume_3=cgs.volume_3 * _cm3,
            density_2=cgs.density_2 * _g_cm3,
            density_3=cgs.density_3 * _g_cm3,
            pressure_2=cgs.pressure_2 * _dyn_cm2,
            pressure_3=cgs.pressure_3 * _dyn_cm2,
            temperature_2=cgs.temperature_2 * u.K,
            temperature_3=cgs.temperature_3 * u.K,
            thermal_energy_density_2=cgs.thermal_energy_density_2 * _erg_cm3,
            thermal_energy_density_3=cgs.thermal_energy_density_3 * _erg_cm3,
            sound_speed_2=cgs.sound_speed_2 * _cm_s,
            sound_speed_3=cgs.sound_speed_3 * _cm_s,
            post_shock_density_rs=cgs.post_shock_density_rs * _g_cm3,
            post_shock_pressure_rs=cgs.post_shock_pressure_rs * _dyn_cm2,
            post_shock_temperature_rs=cgs.post_shock_temperature_rs * u.K,
            post_shock_thermal_energy_density_rs=cgs.post_shock_thermal_energy_density_rs * _erg_cm3,
            post_shock_density_fs=cgs.post_shock_density_fs * _g_cm3,
            post_shock_pressure_fs=cgs.post_shock_pressure_fs * _dyn_cm2,
            post_shock_temperature_fs=cgs.post_shock_temperature_fs * u.K,
            post_shock_thermal_energy_density_fs=cgs.post_shock_thermal_energy_density_fs * _erg_cm3,
        )
    ...
    def _compute_shock_properties_cgs(
        ...
        **kwargs,
        ...
        r"""
        Integrate the mechanical shock ODEs in CGS units and return all diagnostics.

        When ``M1_total`` is finite the integration is split at the ejecta-crossing
        event to restart Radau cleanly at the RHS discontinuity, avoiding unnecessary
        step-size reduction near the switch.
        """
        time = np.atleast_1d(np.asarray(time, dtype=float))
        ...
        y0 = np.array([R_cd_0, v_cd_0**2, M2_0, M3_0, U2_0, U3_0, Delta2_0, Delta3_0], dtype=float)
        t_max = float(np.amax(time))
        t_span = (t_0, t_max)

        solver_kwargs = dict(kwargs)
        rtol = solver_kwargs.pop("rtol", self._DEFAULT_RTOL)
        method = solver_kwargs.pop("method", "Radau")

        if np.isfinite(_M1_total):
            # Two-phase integration: detect the ejecta-crossing event precisely so
            # Radau can restart with fresh step-size control on the smooth post-crossing
            # segment, rather than hammering small steps across the RHS discontinuity.
            def _crossing(t, y):
                return y[2] - (1.0 - _EJECTA_MASS_TOL) * _M1_total

            _crossing.terminal = True
            _crossing.direction = 1.0

            sol1 = solve_ivp(
                kernel, t_span, y0, t_eval=time, events=[_crossing], rtol=rtol, method=method, **solver_kwargs
            )
            if sol1.status < 0:
                raise RuntimeError(f"ODE solver failed (phase 1):\n{sol1.message}")

            if sol1.t_events[0].size > 0:
                t_cross = sol1.t_events[0][0]
                y_cross = sol1.y_events[0][0].copy()
                # No Delta2 reset: keep Delta2_cross so there is no sudden volume jump
                # in V2 = 4*pi*R_cd^2*Delta2. Post-crossing dwidth2/dt = v_cd (kernel) makes
                # both R_cd and Delta2 grow at v_cd, so R_rs = R_cd - Delta2 stays near
                # zero without a discontinuity in dU_ad2 or dPhi_dt.
                n_before = sol1.y.shape[1]
                after = np.arange(len(time)) >= n_before

                if np.any(after):
                    sol2 = solve_ivp(
                        kernel, (t_cross, t_max), y_cross, t_eval=time[after], rtol=rtol, method=method, **solver_kwargs
                    )
                    if sol2.status < 0:
                        raise RuntimeError(f"ODE solver failed (phase 2):\n{sol2.message}")
                    Y = np.empty((8, len(time)))
                    Y[:, ~after] = sol1.y
                    Y[:, after] = sol2.y
                else:
                    Y = sol1.y
            ...
            sol = solve_ivp(kernel, t_span, y0, t_eval=time, rtol=rtol, method=method, **solver_kwargs)
            if sol.status < 0:
                raise RuntimeError(f"ODE solver failed:\n{sol.message}")
            Y = sol.y

        R_cd, Phi, M2, M3, U2, U3, Delta2, Delta3 = Y
        v_cd = np.sqrt(np.maximum(Phi, 0.0))

        # Geometry. R_rs is floored at 0: after ejecta crossing Delta2 = R_cd so
        # R_rs = 0 exactly, and small numerical residuals are harmless.
        R_rs = np.maximum(R_cd - Delta2, 0.0)
        R_fs = R_cd + Delta3
        _4pi = 4.0 * np.pi
        V2 = _4pi * R_cd**2 * np.minimum(Delta2, R_cd)
        V3 = _4pi * R_cd**2 * Delta3

        # Region-averaged thermodynamics (floor U to prevent negative pressure)
        U2_eff = np.maximum(U2, 0.0)
        U3_eff = np.maximum(U3, 0.0)

        density_2 = M2 / V2
        density_3 = M3 / V3
        pressure_2 = (gamma_2 - 1.0) * U2_eff / V2
        pressure_3 = (gamma_3 - 1.0) * U3_eff / V3
        thermal_energy_density_2 = U2_eff / V2
        thermal_energy_density_3 = U3_eff / V3
        ...
        # Upstream quantities at shock faces for post-processed jump diagnostics
        n_steps = len(time)
        ...
        # Shock kinematics (mirrors ODE kernel logic for consistency).
        # Post-crossing: dDelta2/dt = v_cd (same as kernel) so D_rs = 0, matching
        # the no-RS state.  The outputs R_rs and velocity_rs are masked to 0 / NaN
        # when ejecta_crossed so callers are not misled by the unphysical value.
        _ejecta_crossed = M2 >= (1.0 - _EJECTA_MASS_TOL) * _M1_total
        delta_v_2 = u_1_rs - v_cd
        dDelta2_dt = np.where(
            _ejecta_crossed,
            v_cd,  # kernel branch post-crossing
            np.maximum(delta_v_2 / (chi_2 - 1.0), 0.0),
        )
        D_rs = v_cd - dDelta2_dt  # = 0 when _ejecta_crossed
        D_fs = v_cd + sound_speed_3
        active_rs = (delta_v_2 > 0.0) & ~_ejecta_crossed
        active_fs = (D_fs - u_4_fs) > 0.0

        # Instantaneous RH jump diagnostics (NaN where shock is inactive)
        psd_fs = np.full_like(R_cd, np.nan)
        psp_fs = np.full_like(R_cd, np.nan)
        pst_fs = np.full_like(R_cd, np.nan)
        pste_fs = np.full_like(R_cd, np.nan)
        psd_rs = np.full_like(R_cd, np.nan)
        psp_rs = np.full_like(R_cd, np.nan)
        pst_rs = np.full_like(R_cd, np.nan)
        pste_rs = np.full_like(R_cd, np.nan)

        if np.any(active_fs):
            rh = StrongColdShockConditions._solve(
                D_fs[active_fs],
                rho_4_fs[active_fs],
                u_4_fs[active_fs],
                gamma=gamma_3,
            ...
            psd_fs[active_fs] = rh["post_shock_density"]
            psp_fs[active_fs] = rh["post_shock_pressure"]
            pst_fs[active_fs] = rh["post_shock_temperature"]
            pste_fs[active_fs] = rh["post_shock_thermal_energy_density"]
        ...
        if np.any(active_rs):
            ...
            psd_rs[active_rs] = rh["post_shock_density"]
            psp_rs[active_rs] = rh["post_shock_pressure"]
            pst_rs[active_rs] = rh["post_shock_temperature"]
            pste_rs[active_rs] = rh["post_shock_thermal_energy_density"]

        # Mask RS position and velocity to 0/NaN once the RS no longer exists.
        # The ODE state (Delta2, U2) continues to evolve but R_rs and D_rs
        # are no longer physically meaningful after ejecta crossing.
        R_rs_out = np.where(_ejecta_crossed, 0.0, R_rs)
        D_rs_out = np.where(_ejecta_crossed, np.nan, D_rs)
        ...
        return MechanicalShockState(
            ...
            post_shock_density_rs=psd_rs,
            post_shock_pressure_rs=psp_rs,
            post_shock_temperature_rs=pst_rs,
            post_shock_thermal_energy_density_rs=pste_rs,
            post_shock_density_fs=psd_fs,
            post_shock_pressure_fs=psp_fs,
            post_shock_temperature_fs=pst_fs,
            post_shock_thermal_energy_density_fs=pste_fs,
        )
