...
In this example we model an outflow launched at :math:`v_{\rm init} = 0.3\,c`
with total kinetic energy :math:`E_{\rm ej} = 10^{50}` erg. The ejecta mass
...
.. hint::
    ...
    TDE outflow parameters :math:`E_{\rm ej}` and :math:`n_{\rm ISM}` are
...
# -------------------
#
# We set the launch velocity to :math:`0.3\,c` and kinetic energy to
# :math:`10^{50}` erg, then derive the ejecta mass from the kinetic energy
# relation :math:`E_{\rm ej} = \tfrac{1}{2} M_{\rm ej} v^2`.
...
# (:math:`R \propto t`), then transition to the **Sedov-Taylor** phase
...
# .. math::
#
#     B = \sqrt{8\pi\,\epsilon_B\,e_{\rm th}}.
...
# values of :math:`E_{\rm ej}` while keeping the launch velocity fixed at
# :math:`0.3\,c`, so the ejecta mass scales as :math:`M_{\rm ej} = 2 E_{\rm ej} / v^2`
# for each case.
