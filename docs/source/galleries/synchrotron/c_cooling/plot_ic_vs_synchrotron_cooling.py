...
In the presence of an external radiation field relativistic electrons lose
energy not only through synchrotron radiation but also through **inverse
Compton (IC) scattering** — up-scattering photons to higher energies.  In the
Thomson regime the IC cooling rate has the same :math:`\gamma^2` dependence as
synchrotron cooling, but the energy density driving it is the radiation energy
density :math:`u_{\rm rad}` rather than the magnetic energy density
:math:`u_B = B^2/(8\pi)`:

.. math::

    \left|\frac{dE}{dt}\right|_{\rm IC} = \frac{4}{3}\sigma_T c\, u_{\rm rad}\, \gamma^2,
    \qquad u_{\rm rad} = \frac{L_{\rm bol}}{4\pi R^2 c}

The **Compton Y parameter** measures the relative importance of the two
cooling channels:

.. math::

    Y \equiv \frac{u_{\rm rad}}{u_B} = \frac{L_{\rm bol}}{4\pi R^2 c} \cdot \frac{8\pi}{B^2}

When :math:`Y > 1` IC cooling dominates over synchrotron cooling and the
effective cooling break frequency is suppressed by a factor :math:`(1 + Y)^{-2}`.

This example demonstrates both cooling engines and their combined effect on the
...
# ----------------------------------------------------
#
# We compare the synchrotron cooling rate for :math:`B = 1` G with the IC
# cooling rate for a bolometric luminosity :math:`L_{\rm bol} = 10^{43}`
# erg s\ :math:`^{-1}` at a radius :math:`R = 10^{16}` cm.
#
# The crossover Lorentz factor :math:`\gamma_\times` at which the two rates
# are equal does not depend on :math:`\gamma` (since both go as
# :math:`\gamma^2`); instead it depends on the ratio :math:`u_{\rm rad}/u_B = Y`.
...
# ------------------------------------
#
# The Compton Y parameter can be written as a function of the combined
# observable :math:`L_{\rm bol}/R^2`:
#
# .. math::
#
#     Y = \frac{2 L_{\rm bol}}{\sigma_T c^2 B^2 R^2}
#       \equiv \frac{u_{\rm rad}}{u_B}
#
# We plot :math:`Y` as a function of :math:`L_{\rm bol}/R^2` for three
# magnetic field strengths.  The :math:`Y = 1` line separates the IC-dominated
# and synchrotron-dominated regimes.
...
# -------------------------------------------------------
#
# When IC cooling is significant the total cooling time is
# :math:`t_{\rm cool}^{-1} = t_{\rm synch}^{-1} + t_{\rm IC}^{-1}`, so
# the effective cooling Lorentz factor is reduced:
#
# .. math::
#
#     \gamma_c^{\rm eff} = \frac{\gamma_c^{\rm synch}}{1 + Y}
#
# and the cooling break frequency :math:`\nu_c^{\rm eff} \propto (\gamma_c^{\rm eff})^2 B`
# is suppressed by a factor :math:`(1 + Y)^{-2}` relative to the synchrotron-only
# prediction.
...
# ----------
#
# Inverse Compton cooling is critical in any environment with a significant
# radiation field — for example, in TDEs where the disk luminosity can
# reach the Eddington limit, or in GRB afterglows where the prompt emission
# provides a seed photon field.
#
# Key results from this example:
#
# - Both synchrotron and IC cooling scale as :math:`\gamma^2`, so the ratio
#   of their rates (the Compton Y parameter) is independent of the electron
#   Lorentz factor.  IC simply rescales the effective cooling rate.
# - When :math:`Y > 1` the cooling break is suppressed by :math:`(1 + Y)^{-2}`,
#   which can move :math:`\nu_c` below the radio band even when the synchrotron-
#   only estimate predicts :math:`\nu_c` in the optical or X-ray.
# - For GHz radio sources this can qualitatively change the spectral regime
#   (from slow-cooling to fast-cooling) and must be accounted for in closure
#   relation analyses.
#
# This implementation assumes the Thomson regime.  Klein–Nishina corrections
# become important when :math:`4\gamma E_{\rm ph}/(m_e c^2) \gtrsim 1` and
# reduce the effective IC cross-section, partially restoring the synchrotron
# prediction for very high-:math:`\gamma` electrons.
